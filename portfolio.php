...
if ($_POST['sellstocks']){
	$result = DB::run("SELECT * FROM `stocks` WHERE `id`='".$_POST['stocks_id']."'");
    $worked = $result->fetch();
    $price = $worked['cost'];
...
<?php
$result = DB::run("SELECT * FROM `shares` WHERE `userid`='".$user_class->id."' ORDER BY `userid` ASC");
while($line = $result->fetch(PDO::FETCH_ASSOC)) {
	$result2 = DB::run("SELECT * FROM `stocks` WHERE `id`='".$line['companyid']."'");
