...
$checkeffects = DB::run("SELECT * FROM `effects` WHERE `userid`='$user_class->id'");
$numeffects = $checkeffects->rowCount();
if($numeffects > 0){
...
if ($_POST['energy'] < 1){
  ...
  die();
	...
	if($_POST['type'] == 1){ // strength
		...
		$result =  DB::run("UPDATE `grpgusers` SET `strength` = '".$newstrength."' WHERE `id` = '".$user_class->id."'");
		...
		$result =  DB::run("UPDATE `grpgusers` SET `defense` = '".$newdefense."' WHERE `id` = '".$user_class->id."'");
		...
		$result =  DB::run("UPDATE `grpgusers` SET `speed` = '".$newspeed."' WHERE `id` = '".$user_class->id."'");
...
$result =  DB::run("UPDATE `grpgusers` SET `awake` = '".$newawake."', `energy` = '".$newenergy."' WHERE `id` = '".$user_class->id."'");
