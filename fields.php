...
if ($_GET['harvest']){
    $result = DB::run("SELECT * FROM `growing` WHERE `id`='".$_GET['harvest']."'");
    $worked = $result->fetch();
    ...
    $result = DB::run("UPDATE `grpgusers` SET `marijuana` = '".$newpot."' WHERE `id`='".$user_class->id."'");
    Give_Land($worked['cityid'], $worked['userid'], $worked['amount']);
    $user_class = new User($_SESSION['id']);
    $result2= DB::run("DELETE FROM `growing` WHERE `id`='".$_GET['harvest']."'");
    echo Message("You have received ".$worked['cropamount']." ounces of marijuana.");
}

if ($_POST['plant']){
    $result = DB::run("SELECT * FROM `land` WHERE `city`='".$user_class->city."' AND `userid`='".$user_class->id."'");
    $worked = $result->fetch();
    ...
    $result= DB::run("INSERT INTO `growing` (`userid`, `cityid`, `amount`, `croptype`, `cropamount`, `timeplanted`, `timedone`)"."VALUES ('".$user_class->id."', '".$user_class->city."', '".$_POST['amount']."', 'pot', '".($_POST['amount'] * 100)."', ".time().", '".(time() + 604800)."')");

    $newland = $worked['amount'] - $_POST['amount'];
    $newpotseeds = $user_class->potseeds - ($_POST['amount'] * 100);
    $result = DB::run("UPDATE `grpgusers` SET `potseeds` = '".$newpotseeds."' WHERE `id`='".$user_class->id."'");
...
if ($worked['amount'] > 0){
    ...
    <form method='post'><input type='text' name='amount' size='3' maxlength='20'> Acres <input type='submit' name='plant' value='Plant Pot Seeds'></form>
    <?php
} else {
...
$result = DB::run("SELECT * FROM `growing` WHERE `cityid`='".$user_class->city."' AND `userid`='".$user_class->id."'");
$worked = $result->fetch();
