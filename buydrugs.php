...
if ($_GET['sell'] != "") {
	...
	$newsql = DB::run("UPDATE `grpgusers` SET `money` = '".$newmoney."', `marijuana`='0' WHERE `id`= '".$user_class->id."'");
...
if ($_GET['buy'] != "") {
	...
	if($_GET['buy'] == "potseeds"){
		...
		$newsql = DB::run("UPDATE `grpgusers` SET `potseeds` = '".$newamount."' WHERE `id`= '".$user_class->id."'");
	}
}
