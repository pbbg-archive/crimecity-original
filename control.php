...
$result = DB::run("SELECT * FROM `grpgusers` ORDER BY `lastactive` DESC");
	while($line = $result->fetch(PDO::FETCH_ASSOC)) {
		...
		if ($secondsago > 2592000) {
			...
			$result2 = DB::run("DELETE FROM `grpgusers` WHERE `id`='".$user_online->id."'");
		}
	}
...
//referrals section
if ($_GET['givecredit'] != ""){
	$result = DB::run("UPDATE `referrals` SET `credited`='1' WHERE `id`='".$_GET['givecredit']."'");
	$result = DB::run("SELECT * FROM `referrals` WHERE `id` = '".$_GET['givecredit']."'");
	$line = $result->fetch(PDO::FETCH_ASSOC);
	$cp_user = new User($line['referrer']);
	$newpoints = $cp_user->points + 10;
	$result = DB::Run("UPDATE `grpgusers` SET `points` = '".$newpoints."' WHERE `id`='".$cp_user->id."'");
...
if ($_GET['denycredit'] != ""){
	$result = DB::run("DELETE FROM `referrals` WHERE `id`='".$_GET['denycredit']."'");
...
//jobs section
if ($_GET['deletejob']){
	$result = DB::run("DELETE FROM `jobs` WHERE `id`='".$_GET['deletejob']."'");
...
if ($_POST['addjobdb']){
	$result= DB::run("INSERT INTO `jobs` (name, money, strength, defense, speed, level)"."VALUES ('".$_POST['name']."','".$_POST['money']."','".$_POST['strength']."','".$_POST['defense']."','".$_POST['speed']."', '".$_POST['level']."')");
	echo Message("You have added a job to the database.");
}
if ($_POST['editjobdb']){
	$result= DB::run("UPDATE `jobs` SET `name`='".$_POST['name']."', `money`='".$_POST['money']."', `strength`='".$_POST['strength']."', `defense`='".$_POST['defense']."', `speed`='".$_POST['speed']."', `level`='".$_POST['level']."' WHERE `id`='".$_POST['id']."'");
	echo Message("You have edited a job.");
}
//city section
if ($_GET['deletecity']){
	$result = DB::run("DELETE FROM `cities` WHERE `id`='".$_GET['deletecity']."'");
...
if ($_POST['addcitydb']){
	$result= DB::run("INSERT INTO `cities` (name, levelreq, landleft, landprice, description)"."VALUES ('".$_POST['name']."','".$_POST['levelreq']."','".$_POST['landleft']."','".$_POST['landprice']."','".$_POST['description']."')");
	echo Message("You have added a city to the database.");
}
if ($_POST['editcitydb']){
	$result= DB::run("UPDATE `cities` SET `name`='".$_POST['name']."', `levelreq`='".$_POST['levelreq']."', `landleft`='".$_POST['landleft']."', `landprice`='".$_POST['landprice']."', `description`='".$_POST['description']."' WHERE `id`='".$_POST['id']."'");
	echo Message("You have edited a city.");
}
//crime section
if ($_GET['deletecrime']){
	$result = DB::run("DELETE FROM `crimes` WHERE `id`='".$_GET['deletecrime']."'");
...
if ($_POST['addcrimedb']){
	$result= DB::run("INSERT INTO `crimes` (name, nerve, stext, ftext, ctext)"."VALUES ('".$_POST['name']."','".$_POST['nerve']."','".$_POST['stext']."','".$_POST['ftext']."','".$_POST['ctext']."')");
	echo Message("You have added a crime to the database.");
}
if ($_POST['editcrimedb']){
	$result= DB::run("UPDATE `crimes` SET `name`='".$_POST['name']."', `nerve`='".$_POST['nerve']."', `stext`='".$_POST['stext']."', `ftext`='".$_POST['ftext']."', `ctext`='".$_POST['ctext']."' WHERE `id`='".$_POST['id']."'");
	echo Message("You have edited a crime.");
}
//items section
if ($_POST['additemdb']){
	$result= DB::run("INSERT INTO `items` (itemname,description,cost,image,awake,offense,defense,heal,buyable,level)"."VALUES ('".$_POST['itemname']."','".$_POST['description']."','".$_POST['cost']."','".$_POST['image']."','".$_POST['awake']."','".$_POST['offense']."','".$_POST['defense']."','".$_POST['heal']."','".$_POST['buyable']."','".$_POST['level']."')");
}
if ($_GET['takealluser'] != ""){
	$oldamount = Check_Item($_GET['takeallitem'], $_GET['takealluser']);
	$result = DB::run("DELETE FROM `inventory` WHERE `userid` = '".$_GET['takealluser']."' AND `itemid` = '".$_GET['takeallitem']."'");
...
if ($_POST['listitems'] != ""){
	$oldamount = Check_Item($_POST['itemnumber'], Get_ID($_POST['username']));
	$result = DB::run("SELECT * FROM `inventory` WHERE `userid`='".Get_ID($_POST['username'])."'");
	while($line = $result->fetch(PDO::FETCH_ASSOC)) {
...
if ($_POST['changemessage'] != ""){
	$result = DB::run("UPDATE `serverconfig` SET `messagefromadmin` = '".$_POST['message']."'");
	echo Message("You have changed the message from the admin.");
}

if ($_POST['changeserverdown'] != ""){
	$result = DB::run("UPDATE `serverconfig` SET `serverdown` = '".$_POST['message']."'");
	echo Message("You have changed the server down text.");
}
if ($_POST['addrmdays'] != ""){
	$result = DB::run("SELECT * FROM `grpgusers` WHERE `username`='".$_POST['username']."'");
    $worked = $result->fetch(PDO::FETCH_ASSOC);
	
	$newrmdays = $worked['rmdays'] + $_POST['rmdays'];
	$result = DB::run("UPDATE `grpgusers` SET `rmdays` = '".$newrmdays."' WHERE `username`='".$_POST['username']."'");
...
if ($_POST['addpoints'] != ""){
	...
	$result = DB::run("UPDATE `grpgusers` SET `points` = '".$newpoints."' WHERE `username`='".$_POST['username']."'");
...
if ($_POST['addhookers'] != ""){
	...
	$result = DB::run("UPDATE `grpgusers` SET `hookers` = '".$newhookers."' WHERE `username`='".$_POST['username']."'");
...
if ($_POST['givermgun'] != ""){
    ...
    $result= DB::run("INSERT INTO `inventory` (userid, itemid)".
    "VALUES ('".$worked['id']."', '15')");	
...
if ($_GET['action'] == "deleteallfromip"){
	$result = DB::run("DELETE FROM `grpgusers` WHERE ip='".$_GET['ip']."'");
}
...
$query = "UPDATE grpgusers SET admin = 1 WHERE username = '$user'";
    DB::run($query);
//    or die("Failure to Update a player with Admin Status. MySQL reports: ".mysql_error());
}
}
...
<?php if($_GET['page'] == "") { ?>
	<p>STAFF LINKS</p>
			<font color="black">  <a class="leftmenu" href="control.php" style="font-size:16px">Marquee/Maintenance</a></br>
			 <font color="black"> <a class="leftmenu" href="control.php?page=rmoptions" style="font-size:16px">RM Options</a></font></br>
			<font color="black">  <a class="leftmenu" href="control.php?page=setplayerstatus" style="font-size:16px">Player Options</a></font></br>
			<font color="black">  <a class="leftmenu" href="massmail.php" style="font-size:16px">Mass Mail</a></font></br>
			<font color="black">  <a class="leftmenu" href="control.php?page=referrals" style="font-size:16px">Manage Referrals</a></font></br>
			<font color="black">  <a class="leftmenu" href="control.php?page=crimes" style="font-size:16px">Manage Crimes</a></font></br>
			 <font color="black"> <a class="leftmenu" href="control.php?page=cities" style="font-size:16px">Manage Cities</a></font></br>
			<font color="black">  <a class="leftmenu" href="control.php?page=jobs" style="font-size:16px">Manage Jobs</a></font></br>
			<font color="black">  <a class="leftmenu" href="control.php?page=playeritems" style="font-size:16px">Manage Items</a></font></br>
...
$worked = $result->fetch();
?>
<textarea name='message' cols='53' rows='7'><?php $worked['messagefromadmin']; ?></textarea><br />
...
<input type='text' name='awake' size='10' maxlength='75'> [awake]<br />
...
<tr><td class="contentcontent">
<?php
$result = DB::run("SELECT * FROM `referrals` WHERE `credited`='0'");
while($line = $result->fetch(PDO::FETCH_ASSOC)) {
...
if ($_GET['page'] == "crimes") { ?>
	...
	<tr><td class="contentcontent">
	<?php
	$result = DB::run("SELECT * FROM `crimes`");
	...
	if($_POST['vieweditcrime']){
		$result = DB::run("SELECT * FROM `crimes` WHERE `id`='".$_POST['crimeid']."'");
		$worked = $result->fetch();
		?>
...
<tr><td class="contentcontent">
<?php
$result = DB::run("SELECT * FROM `cities`");
...
<?php
	if($_POST['vieweditcity']){
		$result = DB::run("SELECT * FROM `cities` WHERE `id`='".$_POST['cityid']."'");
		$worked = $result->fetch();
		?>
...
if ($_GET['page'] == "jobs") { ?>
	...
	<tr><td class="contentcontent">
	<?php
	$result = DB::run("SELECT * FROM `jobs`");
	...
	if($_POST['vieweditjob']){
		$result = DB::run("SELECT * FROM `jobs` WHERE `id`='".$_POST['jobid']."'");
		$worked = $result->fetch();
		?>
