...
if ($_GET['spend'] == "energy"){
	if($user_class->points > 9) {
	 $newpoints = $user_class->points - 10;
	  $result = DB::run("UPDATE `grpgusers` SET `energy` = '".$user_class->maxenergy."', `points`='".$newpoints."' WHERE `id`='".$_SESSION['id']."'");
...
if ($_GET['spend'] == "nerve"){
	if($user_class->points > 9) {
	 $newpoints = $user_class->points - 10;
	  $result = DB::run("UPDATE `grpgusers` SET `nerve` = '".$user_class->maxnerve."', `points`='".$newpoints."' WHERE `id`='".$_SESSION['id']."'");
