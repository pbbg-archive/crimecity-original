...
if($_POST['create'] != ""){ // if they are wanting to start a new gang
	...
	$check = DB::run("SELECT * FROM `gangs` WHERE `name`='".$_POST['name']."'");
	$exist = $check->rowCount();
	$error .= ($exist > 0) ? "<div>The Gang Name you chose is already taken.</div>" : "";
	//check if tag is taken yet
	$check = DB::run("SELECT * FROM `gangs` WHERE `tag`='".$_POST['tag']."'");
	$exist = $check->rowCount();
	$error .= ($exist > 0) ? "<div>The tag you chose is already taken.</div>" : "";

	if($error == ""){ // if there are no errors, make the gang
		$result= DB::run("INSERT INTO `gangs` (name, tag, leader)"."VALUES ('".$_POST['name']."', '".$_POST['tag']."', '$user_class->username')");

		$newmoney = $user_class->money - 50000; //deduct the cost of the money
        $result = DB::run("SELECT * FROM `gangs` WHERE `leader` = '".$user_class->username."'");
		$worked = $result->fetch();
		$gangid = $worked['id'];

		$result = DB::run("UPDATE `grpgusers` SET `gang` = '$gangid', `money` = '".$newmoney."' WHERE `id`='".$_SESSION['id']."'");
