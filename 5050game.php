...
if ($_POST['takebet'] != ""){
    ...
    $worked = DB::run("SELECT * FROM `5050game` WHERE `id`='".$_POST['bet_id']."'")->fetch();
//    $worked = mysql_fetch_array($result);
	...
	if($amount <= $user_class->money){
		...
		$result = DB::run("DELETE FROM `5050game` WHERE `id`='".$worked['id']."'");
...
if ($_POST['makebet']){
	...
	if($_POST['amount'] >= 1000 && $_POST['amount'] <= $user_class->money){
		...
		$result= DB::run("INSERT INTO `5050game` (owner, amount)"."VALUES ('$user_class->id', '$_POST[amount]')");
...
<tr><td class="contentcontent">
<?php
$result = DB::run("SELECT * FROM `5050game` ORDER BY `amount` DESC");
