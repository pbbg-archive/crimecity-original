...
if ($_POST['invite'] != ""){
		...
		$checkuser2 = DB::run("SELECT `username` FROM `ganginvites` WHERE `username`='".$to."' AND `gangid`='".$gang_class->id."'");
		$username_exist2 = $checkuser2->rowCount();
		...
		if($username_exist > 0 && $username_exist2 == 0){
		  $result= DB::run("INSERT INTO `ganginvites` (`username`, `gangid`)".
		  "VALUES ('$to', '$gang')");
