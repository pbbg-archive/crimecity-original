...
while($yourhp > 0 && $theirhp > 0){
	...
	if($theirhp <= 0){ // attacker won
		 ...
		 $result = DB::run("UPDATE `grpgusers` SET `exp` = '".$newexp."', money = '".$newmoney."', `battlewon` = '".$battlewon."', `battlemoney` = '".$battlemoney."' WHERE `id`='".$_SESSION['
		 ...
		 $result = DB::run("UPDATE `grpgusers` SET `money` = '".$newmoney."', `hwho` = '".$user_class->username."', `hhow` = 'wasattacked', `hwhen` = '".date(g.":".i.":".sa,time())."', `hospital` = '1200', `battlelost` = '".$battlelost."', `battlemoney` = '".$battlemoney."' WHERE `id`='".$attack_person->id."'");
		 ...
		 if ($user_class->gang != 0) {
			 ...
			 $result = DB::run("UPDATE `gangs` SET `exp` = '".$newgangexp."' WHERE `id`='".$gang->id."'");
		 }
	}
	...
	if($yourhp <= 0){ // defender won
		 ...
		 $newmoney = $attack_person->money + $moneywon;
		 $result = DB::run("UPDATE `grpgusers` SET `exp` = '".$newexp."', money = '".$newmoney."', `battlewon` = '".$battlewon."', `battlemoney` = '".$battlemoney."' WHERE `id`='".$attack_person->id."'");

		 $newmoney = $user_class->money - $moneywon;
		 $battlelost = $user_class->battlelost + 1;
		 $battlemoney = $user_class->battlemoney - $moneywon;
		 $result = DB::run("UPDATE `grpgusers` SET `money` = '".$newmoney."', `hwho` = '".$attack_person->username."', `hhow` = 'attacked', `hwhen` = '".date(g.":".i.":".sa,time())."', `hospital` = '1200', `battlelost` = '".$battlelost."', `battlemoney` = '".$battlemoney."' WHERE `id`='".$user_class->id."'");
...
if ($attack_person->gang != 0) {
	$time = time();
	$result= DB::run("INSERT INTO `ganglog` (`timestamp`, gangid, attacker, defender, winner)"."VALUES ('".$time."', '".$attack_person->gang."', '".$user_class->id."', '".$attack_person->id."', '".$winner."')");
}
//update users
$newenergy = $user_class->energy - floor($user_class->energy * .10);
$result = DB::run("UPDATE `grpgusers` SET `hp` = '".$theirhp."' WHERE `id`='".$attack_person->id."'");
$result = DB::run("UPDATE `grpgusers` SET `hp` = '".$yourhp."', `energy` = '".$newenergy."' WHERE `id`='".$user_class->id."'");
echo "</td></tr>";
include 'footer.php';
?>