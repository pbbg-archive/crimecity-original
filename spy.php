...
if ($_GET['id'] != ""){
    ...
    if($_GET['confirm'] != "yes"){
        ...
        if($cost > $user_class->money){
            ...
            echo "Your Private Investigator found out the following about ".$spy_class->formattedname.":<br>Strength - ".prettynum($strength)." <br> Defense - ".prettynum($defense)." <br> Speed - ".prettynum($speed)." <br> Bank - ". prettynum($bank,1) ."<br> Points - ". prettynum($points). "<br><br><center><a href='NEW/spylog.php'>View Spylog</a></center>";
            $total = $user_class->money - $cost;
            $result = DB::run("UPDATE `grpgusers` SET `money` = '".$total."' WHERE `id` = '".$user_class->id."'");
            ...
            $result=DB::run("INSERT INTO `spylog` (`id`, `spyid`, `strength`, `defense`, `speed`, `bank`, `points`, `age`) VALUES ('".$user_class->id."', '".$spy_class->id."', '".$strength."', '".$defense."', '".$speed."', '".$bank."', '".$points."', '".time()."')");
