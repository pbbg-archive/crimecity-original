...
if($_POST['deposit'] != ""){
    ...
    if ($amount <= $user_class->money && $amount > 0) {
        ...
        $result = DB::run("UPDATE `grpgusers` SET `bank` = '".$newbank."', `money` = '".$newmoney."' WHERE `id`='".$_SESSION['id']."'");
...
if($_GET['open'] == "new"){
    if($user_class->money >= 5000 && $user_class->bank == 0){
     $newmoney = $user_class->money - 5000;
        $result = DB::run("UPDATE `grpgusers` SET `whichbank` = '1', `money` = '".$newmoney."' WHERE `id`='".$_SESSION['id']."'");
