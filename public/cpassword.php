...
if (isset($_POST['submit'])) {
  ...
  $checkuser = DB::run("SELECT * FROM `grpgusers` WHERE `password`='$oldpassword' AND `id`='".$user_class->id."'");

  $user_exist = $checkuser->rowCount();
  ...
  //insert the values
  if (!isset($message)){
    $result= DB::run("UPDATE `grpgusers` SET `password`='".$password."' WHERE `id`='".$user_class->id
	."'");
