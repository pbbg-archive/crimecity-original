...
if (isset($_POST['train'])) {
    ...
    if($_POST['type'] == 1) { // strength
        ...
        $result =  DB::run("UPDATE `grpgusers` SET `strength` = ? WHERE `id` = ?", [$newstrength, $user_class->id]);
        ...
        $result =  DB::run("UPDATE `grpgusers` SET `defense` = ? WHERE `id` = ?", [$newdefense, $user_class->id]);
        ...
        $result =  DB::run("UPDATE `grpgusers` SET `speed` = ? WHERE `id` = ?", [$newspeed, $user_class->id]);
    ...
    $result =  DB::run("UPDATE `grpgusers` SET `awake` = ?, `energy` = ? WHERE `id` = ?", [$newawake, $newenergy, $user_class->id]);
