...
$error = ($user_class->jail > 0) ? "You can't do crimes if you are in jail." : null;
...
if ($crime) {
    ...
    if (!$worked) {
        echo Message('This crime does not exist.');
        include 'footer.php';
        die();
    }
	...
	if ($user_class->nerve >= $nerve) {
		if ($chance <= 75) {
			...
			$result = DB::run("UPDATE `grpgusers` SET `exp` = ?, `crimesucceeded` = ?, `crimemoney` = ?, `money` = ?, `nerve` = ? WHERE `id` = ?", [
			    $exp,
                $crimesucceeded,
                $crimemoney,
                $money,
			...
			$jail = $crime * 60 * 10;
			$nerve = $user_class->nerve - $nerve;
			$result = DB::run("UPDATE `grpgusers` SET `crimefailed` = ?, `jail` = ?, `nerve` = ? WHERE `id` = ?", [
			    $crimefailed,
                $jail,
                $nerve,
                $_SESSION['id']
			...
			$crimefailed = 1 + $user_class->crimefailed;
			$nerve = $user_class->nerve - $nerve;
			$result = DB::run("UPDATE `grpgusers` SET `crimefailed` = ?, `nerve` = ? WHERE `id` = ?", [
                $crimefailed,
                $nerve,
                $_SESSION['id']
            ]);
