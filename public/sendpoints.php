...
if($_POST['sendpoints'] != ""){
    ...
    if($user_class->points >= $_POST['amount'] && $_POST['amount'] > 0 && $user_class->id != $money_person->id){
        ...
        $result = DB::run("UPDATE `grpgusers` SET `points` = '".$newpoints."' WHERE `id`='".$_SESSION['id']."'");

        $newpoints = $money_person->points + $_POST['amount'];
        $result = DB::run("UPDATE `grpgusers` SET `points` = '".$newpoints."' WHERE `id`='".$_POST['theirid']."'");
...
?>
    ...
    <div class="content"><u>Send Points</u><br>
        <form name='login' class="sendpoints-form" method='post' action='sendpoints.php'>
            <p>&nbsp;</p>
