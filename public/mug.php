...
if ($user_class->speed > $attack_person->speed){
    ...
    $result = DB::run("UPDATE `grpgusers` SET `money` = '".$newmuggeramount."' WHERE `id`='".$user_class->id."'");
    $result =  DB::run("UPDATE `grpgusers` SET `money` = '".$newmuggedamount."' WHERE `id`='".$attack_person->id."'");
} else {
...
$result =  DB::run("UPDATE `grpgusers` SET `nerve` = '".$newnerve."' WHERE `id`='".$user_class->id."'");
