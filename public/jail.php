...
if ($jailed_person->jail == "0"){
	...
	if ($user_class->nerve >= $nerve) {
		if($chance <= 75) {
			...
			$result =  DB::run("UPDATE `grpgusers` SET `exp` = '".$exp."', `crimesucceeded` = '".$crimesucceeded."', `crimemoney` = '".$crimemoney."', `money` = '".$money."', `nerve` = '".$nerve."' WHERE `id`='".$_SESSION['id']."'");
			$result =  DB::run("UPDATE `grpgusers` SET `jail` = '0' WHERE `id`='".$jailed_person->id."'");
			...
			$jail = 10800;
			$nerve = $user_class->nerve - $nerve;
			$result =  DB::run("UPDATE `grpgusers` SET `crimefailed` = '".$crimefailed."', `jail` = '".$jail."', `nerve` = '".$nerve."' WHERE `id`='".$_SESSION['id']."'");
		}else{
			echo Message("You failed.");
			$crimefailed = 1 + $user_class->crimefailed;
			$nerve = $user_class->nerve - $nerve;
			$result =  DB::run("UPDATE `grpgusers` SET `crimefailed` = '".$crimefailed."', `nerve` = '".$nerve."' WHERE `id`='".$_SESSION['id']."'");
		}
	} else {
...
<table width='100%' cellpadding='4' cellspacing='0'>
	...
	<?php
$result =  DB::run("SELECT * FROM `grpgusers` ORDER BY `jail` DESC");
