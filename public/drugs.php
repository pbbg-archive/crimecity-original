...
$numeffects = $checkeffects->rowCount();
	...
	$newsql = DB::run("UPDATE `grpgusers` SET `cocaine` = '".$newamount."' WHERE `id`= '".$user_class->id."'");

	echo Message("You snorted some cocaine.");

  $result= DB::run("INSERT INTO `effects` (`userid`, `effect`, `timeleft`)".
...
$numeffects = $checkeffects->rowCount();
	...
	$newsql = DB::run("UPDATE `grpgusers` SET `genericsteroids` = '".$newamount."' WHERE `id`= '".$user_class->id."'");
...
if ($_GET['use'] == "nodoze") {
	...
	$newsql = DB::run("UPDATE `grpgusers` SET `nodoze` = '".$newamount."', `awake` = '".$newawake."' WHERE `id`= '".$user_class->id."'");
