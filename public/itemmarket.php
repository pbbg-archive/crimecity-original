...
if (isset($_GET['buy'])) {
	...
	if ($price > $user_class->money) {
		...
		$result = DB::run("UPDATE `grpgusers` SET `money` = ? WHERE `id` = ?", [$newmoney, $user_class->id]);

		$newmoney = $user_item->money + $price;
		$result = DB::run("UPDATE `grpgusers` SET `money` = ? WHERE `id` = ?", [$newmoney, $user_item->id]);

		$user_class = new User($_SESSION['id']);

		$result = DB::run("DELETE FROM `itemmarket` WHERE `id` = ?", [$_GET['buy']]);
        // give them the item they bought
		Give_Item($worked2['id'], $user_class->id);
	}
}
?>
