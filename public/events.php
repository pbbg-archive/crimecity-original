...
if ($_GET['deleteall'] != ""){
  $result = DB::run("DELETE FROM `events` WHERE `to`='".$user_class->id."'");
  echo Message("All your events have been deleted.");
}
$result2 = DB::run("UPDATE `events` SET `viewed` = '2' WHERE `to`='".$user_class->id."'");

if ($_POST['delete'] != ""){
  $deleteevent = $_POST['event_id'];
  $result = DB::run("DELETE FROM `events` WHERE `id`='".$deleteevent."'");
...
<?php
$result = DB::run("SELECT * from `events` ORDER BY `timesent` DESC");
